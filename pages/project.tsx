...
export default function Home() {
  ...
  const rowsS = resultS.map((row) => {
    ...
    return (
      <tr key={row.time} className="font-semibold">
        ...
        {row.soil < 3800 ? (
          <td className="text-blue-700">{row.soil} - Wet</td>
        ) : (
          <td className="text-amber-800">{row.soil} - Dry</td>
        )}
        {row.state ? (
          <td className="text-green-600">ON</td>
        ) : (
          <td className="text-red-600">OFF</td>
        )}
      </tr>
    );
  });
  const a3 = data2.esp32req.map((t1) => ({
    ...t1,
    ...data2.esp8266acks.find((t2) => t2.id === t1.id),
  }));
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full py-2 ">
      <Head>
        <title>CSE2035 | Presentation</title>
      ...
      <div className="text-2xl font-bold text-center pb-10">Project Page</div>
